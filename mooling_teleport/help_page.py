from mcdreforged.api.all import RTextList, RText, Serializable, CommandSource
...
help_menu = RTextList(
    ...
    RText(
        "§7  使用§r§c!!mtp powerwash§r或§c!!mtp reset config all§r§7以重置所有插件配置项，不影响命令注册相关配置§r"
    ),
)


class BackMessageMain(Serializable):
    console: str = "玩家执行后，可返回其传送前所在或上次死亡的地点"
    player: str = "返回你传送前所在或上次死亡的地点，前者将在此命令提交后刷新"


class BackMessageDied(Serializable):
    console: str = "玩家执行后，返回其上次死亡的地点"
    player: str = "返回你上次死亡的地点，加上整数可指定要返回到的记录"


class BackMessageDiedList(Serializable):
    console: str = "玩家执行后，向其显示其所有死亡记录"
    player: str = "显示你的历史死亡记录"


def get_back_page(src: CommandSource) -> RTextList:
    message_main = BackMessageMain.player if src.is_player else BackMessageMain.console
    message_died = BackMessageDied.player if src.is_player else BackMessageDied.console
    message_died_list = (
        BackMessageDiedList.player if src.is_player else BackMessageDiedList.console
    )
    back_page = RTextList(
...
def get_warp_page() -> RTextList:
    warp_page = RTextList(
        ...
        RText(
            "[!] 目前数据源仅对接了LocationMarker，需要更多功能可以直接使用该插件的命令！"
        ),
    )
    return warp_page
