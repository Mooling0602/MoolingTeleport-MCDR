...
def get_position(player: str) -> dict:
    ...
    if not isinstance(position, list) or len(position) != 3 or not isinstance(dimension, str):
        psi.tell(player, "[木泠牌传送] 无法获取你的位置数据！请联系管理员处理后台的插件报错。")
        raise RuntimeError("Invalid data received, please use rcon or disable oher similar plugins.")
    result = {
        "position": position,
        "dimension": dimension
    }
