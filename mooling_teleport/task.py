        ...
        with lock:
            ...
            for i in rt.tpp_requests:
                ...
                if (now - i.time).total_seconds() >= rt.config.tpp_requests_timeout:
                    server.tell(src, f"你发出的传送请求因超时而被删除，若有需要请重新发起！当前传送超时为{rt.config.tpp_requests_timeout}秒钟")
                    server.tell(target, f"你接受到的传送请求因超时而被删除。当前传送超时为{rt.config.tpp_requests_timeout}秒钟")
