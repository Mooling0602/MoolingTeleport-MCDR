    ...
    if rt.init_status is True and rt.config.force_rcon is False:
        ...
        if not server.is_server_startup():
            server.logger.warning(
                "有待手动确认的事项，将在服务器启动完成后再发出提醒，避免被日志刷屏干扰。"
            )
...
def init_plugin_rcon(server: PluginServerInterface):
    ...
    server.logger.info(
        "插件默认使用Minecraft Data API以获取玩家的位置数据，但当多个插件同时对同一玩家调用该API时，会出现数据错乱问题。"
    )
    server.logger.info("这个问题无法解决，故只能改用Rcon方案才能避免这种情况。")
    if not server.is_rcon_running():
        server.logger.warning(
            "当前MCDR的Rcon功能不可用，需要使用带有Rcon功能的服务端并正确配置！"
        )
        server.logger.warning(
            "建议使用!!MCDR plg install init_server安装初始化插件，以自动化完成配置，你可能需要安装后重启MCDR和服务器才能生效！"
        )
        server.logger.info(
            "配置好Rcon后，使用!!mtp use_rcon来启用插件内置的Rcon方案支持。"
        )
    else:
        server.logger.info("MCDR的Rcon功能正常！")
        server.logger.info("您可以使用!!mtp use_rcon来启用插件内置的Rcon方案支持。")
        server.logger.info(
            "如果您懒得输入完整命令，可以在30秒内在控制台发送y直接进行确认，这个命令是一次性的。"
        )
...
def on_info(server: PluginServerInterface, info: Info):
    if info.is_from_console and info.content == "y":
        time_now = get_time()
        if (
            confirm_request_time is None
            or (time_now - confirm_request_time).total_seconds() <= 30
        ):
...
def on_player_death(
    server: PluginServerInterface, player: str, event: str, content: list
):
    ...
    server.tell(
        player, get_pfxed_message("已记录您的本次死亡位置，可使用!!back快速返回")
    )
