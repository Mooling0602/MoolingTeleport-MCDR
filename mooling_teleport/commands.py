    ...
    def __init__(self, name: str, node: AbstractNode):
        ...
        for argument_child in node._children:
            self.children.append(
                Node(argument_child._ArgumentNode__name, argument_child)
            )
...
def on_command_back_clear(src: CommandSource, ctx: CommandContext):
    ...
    do_clear = False
    if uuid:
        if uuid not in rt.clear_back_data:
            rt.clear_back_data.append(uuid)
        else:
            do_clear = True
    else:
        if src.player not in rt.clear_back_data:
    ...
    src.reply(
        "如果确认要清除你的数据，请§c再次运行这条命令！§r否则，要反悔的话，可以使用§a!!back clear --cancel§r"
    )
...
def on_command_back_clear_confirm(src: CommandSource, ctx: CommandContext):
    ...
    server.logger.warning(
        f"玩家{src.player}正在清除其（死亡）数据（1/2）：删除回溯用位置缓存（如有）"
    )
    ...
    if os.path.exists(target_path):
        ...
        server.logger.warning(
            f"玩家{src.player}清除了其（死亡）数据（2/2）：{target_path}"
        )
    else:
        server.logger.warning(
            f"玩家{src.player}清除了其（死亡）数据（2/2）：无历史死亡记录，跳过"
        )
...
def on_command_tpa_player(src: CommandSource, ctx: CommandContext):
    ...
    src.reply(
        f"已向玩家{ctx['player']}发送传送请求，对方通过后将把你传送至其所在位置。"
    )
...
def on_command_tph_player(src: CommandSource, ctx: CommandContext):
    ...
    server.tell(
        ctx["player"], f"[玩家间传送] 玩家{src.player}想要把你传送到其所处的位置！"
    )
