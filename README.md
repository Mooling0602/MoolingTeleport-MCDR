...
`!!back` 返回到传送前的位置或上次死亡的地点，需要玩家环境
> 根据传送前的位置缓存情况和时间远近，自动选择要实际执行的子命令（`!!back died` / `!!reback`）
...
> 历史死亡地点会按照时间进行排序，数字越大表示离现在越近
>
> 开发中内容，将于首个正式版后完成：如果玩家发生死亡的坐标点离得很近，则会直接刷新已有记录而不是新增记录，可以在配置文件调整

`!!back teleport` / `!!back tp` / `!!reback` 返回到传送前自己所在的位置，需要玩家环境

`!!back clear [--confirm|--cancel]` 清空自己和传送相关的历史记录，需要玩家环境
> 此操作将直接删除相关数据文件，无法撤销！
>
...
> 使用BukkitChatManager时，暂时无法在聊天消息中隐藏这些指令，后续将进行适配
